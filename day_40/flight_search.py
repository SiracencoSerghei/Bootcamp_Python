...
FLIGHT_OFFERS_ENDPOINT = os.environ.get("FLIGHT_OFFERS_ENDPOINT")
...
class FlightSearch:
    ...
    def _get_new_token(self):
        ...
        if not IATA_ENDPOINT:
            raise ValueError("IATA_ENDPOINTT environment variable is not set")
        if not FLIGHT_OFFERS_ENDPOINT:
            raise ValueError("FLIGHT_OFFERS_ENDPOINT environment variable is not set")
...
if __name__ == "__main__":
    token = FlightSearch()
    print(token._token)
    city = token.get_destination_code("London")
    print(city)
